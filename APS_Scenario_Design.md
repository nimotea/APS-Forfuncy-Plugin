...
## 6. 讨论区
- [x] 是否需要支持“多资源/多机台”排产？ -> **已决定采用“排序+分配”解耦架构，当前分支 `feature/gantt-assignment` 重点攻克资源分配逻辑。**
- [x] 是否需要增加“机台能力差异”约束？ -> **V1.0 暂定同质机台，但在接口设计上预留 ResourceID 字段。**
- [ ] 是否需要 Demo 包含甘特图展示？

---

## 7. 高级特性分支规划 (feature/gantt-assignment)

本分支致力于实现 **APS 资源分配 (Assignment)** 核心逻辑，将“排序后的工单”转化为“甘特图数据”。

### 7.1 核心目标
在服务端插件中实现轻量级的排程计算引擎，输入“有序工单”和“可用资源”，输出精确到分钟的作业计划。

### 7.2 新增命令：`AssignResources` (资源分配)

#### 输入参数 (Input)
1.  **SortedOrders (有序工单列表)**: 
    -   来源：通常由 `Sequencing` 插件输出。
    -   字段：`Id` (工单号), `Duration` (加工时长, 小时/分钟)。
2.  **Resources (资源列表)**:
    -   字段：`ResourceId` (机台号), `Efficiency` (效率系数, 默认1.0)。
    -   *(可选)* `InitialAvailableTime` (初始可用时间，用于接续排产)。
3.  **StartTime (全局计划开始时间)**: 默认为当前时间。

#### 核心算法 (Logic)
采用 **EFT (Earliest Finish Time) 贪心算法**：
1.  遍历每一个待排工单。
2.  遍历所有可用资源，计算该工单在每个资源上的 `预计完成时间 = max(资源可用时间, 全局开始时间) + (工单时长 / 资源效率)`。
3.  选择 `预计完成时间` 最早的资源进行分配。
4.  更新该资源的 `资源可用时间`。

#### 输出结果 (Output)
返回一个 JSON 数组，可直接绑定给甘特图组件：
```json
[
  {
    "TaskId": "WO001",
    "ResourceId": "Machine_A",
    "StartTime": "2026-02-14 09:00:00",
    "EndTime": "2026-02-14 13:30:00"
  },
  {
    "TaskId": "WO002",
    "ResourceId": "Machine_B",
    "StartTime": "2026-02-14 09:00:00",
    "EndTime": "2026-02-14 11:00:00"
  }
]
```
