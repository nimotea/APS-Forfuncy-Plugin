...
## 4. 场景四：多级排序策略 (Multi-level Tie-breaking)

### 场景描述
在实际生产中，单一规则往往无法满足复杂需求。例如：
- **EDD + SPT**：在交货日期（DueDate）相同的情况下，优先安排加工时间（ProcessingTime）短的工单，以加速资金周转。
- **EDD + Priority**：在交货日期相同的情况下，优先安排高优先级的客户订单。

### Demo 实现思路
- **配置方式**：用户可以在设计器中添加多条排序规则。
- **逻辑**：插件将按顺序执行 `OrderBy` -> `ThenBy` -> `ThenBy`...
- **效果**：系统首先满足第一级规则（如 EDD），在第一级规则“平局”时，自动应用第二级规则（如 SPT），从而生成更科学、确定的排程结果。

---

## 5. 未来扩展：资源分配命令 (Assignment)
...
2.  **插件调用**：
    ...
    -   **规则**：配置 `SortRules` 列表（第一级 EDD，第二级 SPT...）。
